...
const monsterMove = document.getElementById('monster-move');
const monsterAttacks = document.getElementById('monster-attacks');
const monsterSpecialAttacks = document.getElementById('monster-special-attacks');
const monsterSpecialQualities = document.getElementById('monster-qualities');
...
//respond to button click by printing monster info to the web page

monsterButton.addEventListener('click', () => {
  const monster = monsterFactory();
  monsterName.innerText = monster.name;
  let size = monster.size;
  size = size.slice(0,1).toUpperCase() + size.slice(1,size.length);
  monsterType.innerText = size + " " + monster.type;
  monsterDice.innerText = "Hit Dice: " + monster.hd;
  let movement = '';
  ...
  monsterMove.innerText = movement;
  let attack = '';
  ...
  monsterAttacks.innerText = attack;
  if (monster.specialAttacks.length > 0) {
    monsterSpecialAttacks.innerText = "Special Attacks: " + monster.specialAttacks.join(', ');
  }
  if (monster.specialQualities.length > 0) {
    monsterSpecialQualities.innerText = "Special Qualities: " + monster.specialQualities.join(', ');
  }
})
